...
with open('ac_smile.pickle', 'rb') as handle:
...
def generator_conv(z,labels):
    ...
    train = ly.fully_connected(
       z_labels, 8 * 8 * 256, activation_fn=tf.nn.leaky_relu,normalizer_fn=ly.batch_norm)
    #train = tf.layers.Dense(z_labels,8*8*256,activation=lrelu)

    train = tf.reshape(train, (-1, 8, 8, 256))
    ##多加一層 並且改 lrelu
...
def dis_conv(img, reuse=False):
    with tf.variable_scope('dis_conv') as scope:
        ...
        class_logit = ly.fully_connected(tf.reshape(
            img, [batch_size, 4*4*512]),2,activation_fn=None)
...
saver.restore(sess,'ac_110epochs')
...
res = np.squeeze(res)
plt.imsave(os.path.join(sys.argv[2],'fig3_3.jpg'),res)

plt.figure()
plt.subplot(2,1,1)
temp = pd.read_csv('ac_gl_1.csv')[['Value']]
plt.xlabel('epochs')
plt.ylabel('g_loss')
plt.plot(temp)
...
plt.subplot(2,1,2)
temp = pd.read_csv('ac_dl_1.csv')[['Value']]
plt.xlabel('epochs')
plt.ylabel('d_loss')
plt.plot(temp)
